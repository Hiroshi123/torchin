...
namespace nn{
  ...
  template<typename T>
  class MSECriterion : public nn::Criterion {
  ...
  template<typename T> MSECriterion<T>::MSECriterion(){};
  
  template<typename T> T MSECriterion<T>::updateOutput(Tensor<T>& input, Tensor<T>& target){
  ...
  template<typename T> Tensor<T>& MSECriterion<T>::updateGradInput(){
